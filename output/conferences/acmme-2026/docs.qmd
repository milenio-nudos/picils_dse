---
pagetitle: "Measuring Digital Self-Efficacy in International Large-Scale Assessments: An International Comparison Between ICILS and PISA"
Author: Daniel Miranda, Ismael Aguayo, Juan Carlos Castillo, Nicolas Tobar y Tomás Urzúa
#bibliography: ../../../input/bib/icils_nudos.bib
#csl: ../../../input/bib/apa.csl

knitr:
  opts_chunk:
    collapse: true
    comment: "#>"
    R.options:
      knitr.graphics.auto_pdf: true
      
format:
  revealjs:
    logo: images/acmme.png
    slide-number: true
    theme: [pp.scss]
    title-slide-attributes:
      visibility: false
    transition: zoom
    transition-speed: slow
# data-background-image: images/cover.jpg
# data-background-size: cover
    auto-play-media: true
editor: visual
---

::::: columns
::: {.column width="10%"}
![](images/facso.jpg)

![](images/acmme.png)

![](images/logo_nudos.jpg)
:::

::: {.column .column-right width="85%"}
<br>

### **Measuring Digital Self-Efficacy in International Large-Scale Assessments: A bi-dimensional comparative scale Between ICILS and PISA**

------------------------------------------------------------------------

Daniel Miranda, Ismael Aguayo, Juan Carlos Castillo, Nicolas Tobar y Tomás Urzúa

#### University of Chile y [Millennium Nucleus on Digital Inequalities and Opportunities](https://nudos.cl).

Segundo Congreso Internacional de la Asociación Chilena de Metodología, Medición y Evaluación, La Serena, Chile. 07, 08 y 09 de enero 2026
:::
:::::

## NUDOS {.scrollable}

```{=html}
<iframe width="1000" height="500" src="https://www.nudos.cl/" title="Webpage example"></iframe>
```

[More information: nudos.cl](https://www.nudos.cl/)

![](images/banner_nudos.jpg){width="80%" fig-align="center"}

![](images/lineas-nudos.png)

## Starting point

![](images/dig-context.jpg)

## Digital self-efficacy and gender

::::: columns
::: {.column width="50%" style="font-size: 40px;"}
"... *expectations about one's capabilities to learn and accomplish tasks in digital technologies and digital environments*, is one of the principal components to promote the formation of digital competences" (Ulffert-Blank & Schmidt, 2022).
:::

::: {.column width="50%"}
![](images/woman-prog.jpg){width="100%"}
:::
:::::

## Bidimensional digital self-efficacy?

::: {.incremental .highlight-last style="font-size: 35px;"}
-   A changing concept: Computacional -\> Internet -\> TICs

-   In the last years ICTs has divided in two types of self-efficacy: general and specialized.

-   Another understanding of ICTs that has gained traction is DigComp based in five competences areas.

-   Cross-country studies operationalize digital self-efficacy in a one-dimensional and two-dimensional manner   
:::

## Country differences?

![](images/countries-dif.jpg)

##  {data-background-color="#5f5758"}

::: {style="font-size: 180%; display: flex; justify-content: center; align-items: flex-start; flex-direction: column; text-align: center"}
*It is possible to identify two dimensions on PISA Digital Self-efficacy measurement?*

*Is the bidimensional model of Digital Self-efficacy equivalent by gender and across countries?*

*Which gender differences exist on Digital Self-efficacy across countries?*
:::

# Method {data-background-color="#5f5758"}

## Participants

::::: columns
::: {.column width="50%" style="font-size: 40px;"}

-   Students ($\bar{x}$ = 13.5 years) from 22 shared countries.   

-   International Computer and Information Literacy Study 2023 (n≈91.000).

-   Programme for International Student Assessment 2022 (n≈183.000).
:::

::: {.column width="50%"}
![](images/icils.png){width="50%"} ![](images/pisa-ocde.jpg){width="70%"}
:::
:::::

## Measures

**ICILS**: How well you can do...?

**PISA**: ¿To what extent *are you able to do* the following tasks when using <digital sources>?

::::: columns
::: {.column width="50%" style="font-size: 30px;"}
**General Self-Efficacy**

ICILS: 10 items; PISA: 8 items

-   Search for and evaluate information online.

-   Share content online.

-   Write or edit text.

-   Create a multi-media presentation.
:::

::: {.column width="50%" style="font-size: 30px; color: #ff3057;"}
**Specialized Self-Efficacy**

ICILS: 3 items; PISA: 6 items

-   Website development.

-   Programming.

-   Computational reasoning.
:::
:::::

## Analytic strategy

::: {.incremental .highlight-last style="font-size: 40px;"}
-   Confirmatory Factor Analizes and re-specification

-   Stability among countries and gender (Multigroup CFA)

-   Country and gender differences
:::

# Results {data-background-color="#5f5758"}

1.  Measurement model
2.  Invariance testing
3.  Mean distributions across countries
4.  Gender differences across countries

## ICILS Model

![](images/icils_model.png)

## PISA Model

![](images/pisa_model.png)

## ICILS Invariance

```{r}
#| label: tbl-invariance-icils
#| tbl-cap: "Country and gender measurement invariance ICILS"

library(here)
library(flextable)
library(officer)
invariance_icils <- readRDS(here("output", "article", "tables", "invariance_table_icils.rds"))

# Obtener número de filas
n_rows <- nrow(invariance_icils$body$dataset)

invariance_icils |>
  set_table_properties(
    width = 1,        
    layout = "autofit"  
  ) |>
  width(width = dim(invariance_icils)$widths / sum(dim(invariance_icils)$widths)) |>
  height(height = 0.6, part = "header") |>  # altura del encabezado
  height(height = 2, part = "body") |>    # altura de cada fila del cuerpo (ajusta este valor)
  valign(valign = "center", part = "all") |>
  fontsize(size = 14, part = "all")  # aumenta el tamaño de fuente también
```

## PISA Invariance

```{r}
#| label: tbl-invariance-pisa
#| tbl-cap: "Country and gender measurement invariance PISA"

invariance_pisa <- readRDS(here("output", "article", "tables", "invariance_table_pisa.rds"))

# Obtener número de filas
n_rows <- nrow(invariance_pisa$body$dataset)

invariance_pisa |>
  set_table_properties(
    width = 1,        
    layout = "autofit"  
  ) |>
  width(width = dim(invariance_pisa)$widths / sum(dim(invariance_pisa)$widths)) |>
  height(height = 0.6, part = "header") |>  # altura del encabezado
  height(height = 2, part = "body") |>    # altura de cada fila del cuerpo (ajusta este valor)
  valign(valign = "center", part = "all") |>
  fontsize(size = 14, part = "all")  # aumenta el tamaño de fuente también

```

## 

![](images/icils_country_dse_comparison.png){width="100%"}

## 

![](images/pisa_country_dse_comparison.png){width="100%"}

## 

![](images/icils_gender_dse_gaps.png){width="100%"}

## 

![](images/pisa_gender_dse_gaps.png){width="100%"}

## Conclusion

::: {.incremental .highlight-last style="font-size: 100%;"}
-   Is it possible to identify two dimensions in ICILS and PISA?

    -   Yes, but with fewer items than those proposed by both studies.

-   Is the two-dimensional model of digital self-efficacy equivalent across genders and countries?

    -   In PISA, yes; in ICILS, only at the metric level.

-   What gender differences exist in digital self-efficacy across countries?

    -   Women have an advantage in general self-efficacy, while men in specialized self-efficacy.
:::

## Discussion

::: {.highlight-last style="font-size: 40px;"}
-   The evolution of measurements: ICTSE vs. DSE and the DigComp framework

-   Are there two or more dimensions?

-   The counterintuitive relationship between countries' development and the gender gap
:::

## Next steps {style="font-size: 60px;"}

-   What macro-level factors can explain the differences in digital self-efficacy and the gender gap?

-   What is happening in developed/developing countries?

# Thank you!

-   **Sitio web NUDOS:** [www.nudos.cl](https://www.nudos.cl/)
-   **Repository Github of the proyect:** <https://github.com/milenio-nudos/picils_dse>

# Anex

## Eliminated items

-   Multimedia manipulation
-   Searching for information sources
-   Privacy settings
-   App selection.